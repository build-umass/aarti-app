...
- `quiz-answer` — selecting an option saves the answer and increments the completion counters.
...
- **Answer.** Re-snapshot and click the ref on `Paris` (correct answer), e.g. `agent-browser click "@e21"` with YOUR ref. Then `agent-browser eval "document.body.innerText.match(/Questions completed: \d+\/\d+/)?.[0]"` must report `1/4`.
